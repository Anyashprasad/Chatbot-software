...
from model.chatbot_model import enhanced_chatbot_response
import argparse
...
# ... (rest of your aivoice.py code) ...

@app.route('/voice', methods=['POST'])
def voice():
    allow_general = request.json.get("allow_general", True)
    try:
        result = handle_voice_query(allow_general_knowledge=allow_general)
        return jsonify(result)
    except Exception as e:
        return jsonify({"response": f"Voice error: {e}", "query": ""}), 500

if __name__ == '__main__':
    # start background listener as before
    t = threading.Thread(target=worker, daemon=True)
    t.start()
    stop = recognizer_bg.listen_in_background(mic_bg, callback)
    app.run(debug=True)
